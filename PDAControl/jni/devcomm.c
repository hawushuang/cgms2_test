                   ...
                   devcomm_int t_PacketLength )
{
	...
	for (i = 0; i < DEVCOMM_LINK_COUNT_MAX; i++)
	{
		if ( ( (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_VALID) != 0) && (tp_Link->t_Address == t_Address) ) ||
                      ...
                      devcomm_int *tp_Mode )
{
	...
	*tp_Length -= t_DataLength;
	...
	while ( (REG_GET_BIT(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_ENDING) == 0) && (*tp_Length > 0) )
	{
...
static uint DevComm_CheckDevice( devcomm_int t_Device )
{
	...
	if (REG_GET_BIT(m_t_Control[t_Device].t_Flag, DEVCOMM_CONTROL_FLAG_VALID) == 0)
		return FUNCTION_FAIL;
                                   ...
                                   devcomm_int t_Length )
{
	...
	else
	{
		...
		if (REG_READ_FIELD(tp_ReceivePacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) !=
			REG_READ_FIELD(tp_PacketHead->u8_Control,        PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK)   )
			return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
	...
	if (REG_GET_BIT(tp_Link->tp_ReceivePacketHead->u8_Control, PACKET_CONTROL_OFFSET_ENDING) != 0)
                                 ...
                                 devcomm_link *tp_Link )
{
	...
	while (REG_GET_BIT(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_ENDING) == 0)
	{
		tp_PacketHead = (devcomm_packet_head *)((uint8 *)tp_PacketHead + tp_PacketHead->t_Length);
		u16_Checksum = tp_Callback->fp_GetCRC16( u8p_Buffer,
	...
	if (t_PayloadLength > 0)
	{
		...
		if ( (REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) != tp_Link->u8_ReceiveIdentity) ||
			 (REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK) == 0)                             )
		{
			tp_Link->u8_ReceiveIdentity = REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK);
                                      ...
                                      devcomm_int t_Length )
{
	...
	tp_PacketHead->u8_Control = REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY, PACKET_IDENTITY_MASK);
