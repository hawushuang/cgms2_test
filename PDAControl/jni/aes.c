...
static uint8_t KEY[KEYLEN] = {0};
static const uint8_t* Key;

// Initial Vector used only for CBC mode
static uint8_t IV[KEYLEN] = {0};
...
/* Private functions:                                                        */
/*****************************************************************************/
static void AES_memCpy(uint8_t *dest, const uint8_t *src, uint32_t length)
{
    while (length-- > 0)
        *dest++ = *src++;
}
...
{
    if (u8p_Key!=0)
        AES_memCpy(KEY, u8p_Key, KEYLEN);
    if (u8p_Iv!=0)
        AES_memCpy(IV, u8p_Iv, KEYLEN);
}
...
void AES_CFB_encrypt(uint8_t* buffer, uint8_t length)
{
  ...
  for (i = 0; i < length; i += BLOCKLEN)
  {
    state = (state_t*)iv;
    Cipher();
    for (j = 0; j < BLOCKLEN && i+j < length; ++j)
    {
      buffer[j] ^= iv[j];
      iv[j] = buffer[j];
    }
